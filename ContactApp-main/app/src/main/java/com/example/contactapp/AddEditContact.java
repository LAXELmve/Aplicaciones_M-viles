...
public class AddEditContact extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (isEditMode){
            //set toolbar title
            actionBar.setTitle("Editar Personaje");
            ...
            nameEt.setText(name);
            if (phone.equals(null) || phone.equals("") || phone.equals("null")) {
                phoneEt.setText("");
            } else {
            ...
            if (image.equals(null) || image.equals("") || image.equals("null")){
                Log.d("Edición", "se detectó como nulo");
                profileIv.setImageResource(R.drawable.ic_baseline_person_24);
                //profileIv.setBaseline(R.drawable.ic_baseline_person_24);
            }else {
                Log.d("Edición", "se detectó lleno");
                profileIv.setImageURI(imageUri);
            }

        }else {
            // add mode on
            actionBar.setTitle("Añadir Personaje");
        }
...
//       ContentValues for image info
    ...
    private void saveData() {
        ...
        if (!name.isEmpty()  || !email.isEmpty() ){
            ...
            if (isEditMode){
                // edit mode
                Uri imageUriOld = Uri.parse(image);

                if (imageUri == imageUriOld){
            ...
            // show toast message
            Toast.makeText(getApplicationContext(), "Llena el campo nombre y franqucia", Toast.LENGTH_SHORT).show();
        }
        finish();
    }
            ...
            case CAMERA_PERMISSION_CODE:
                if (grantResults.length >0){
                    ...
                    if (cameraAccepted && storageAccepted){
                        ...
                        Toast.makeText(getApplicationContext(), "Se requieren los permisos a Camara y Almacenamiento..", Toast.LENGTH_SHORT).show();
                    }
                }
                break;
            ...
            case STORAGE_PERMISSION_CODE:
                if (grantResults.length >0){
                    ...
                    if (storageAccepted){
                        ...
                        //permission not granted
                        Toast.makeText(getApplicationContext(), "Se requiere permiso al Almacenamiento..", Toast.LENGTH_SHORT).show();
                    }
                }
                break;
        }
